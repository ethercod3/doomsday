...
Текущий основной сценарий: упаковать файл или уже готовый архив в новый зашифрованный `7z`-архив с шифрованием заголовков и сохранить рядом контрольную сумму `SHA-256`.
...
- Не храните пароль рядом с архивом, в Git, в `.txt`-файлах или в истории команд, если устройство может быть скомпрометировано.
- Не передавайте пароль тем же каналом, что и архив.
- Используйте длинные парольные фразы, а не короткие числовые пароли.
- Не переиспользуйте один пароль для разных передач.
- После передачи удаляйте временные незашифрованные копии, если они больше не нужны.

Этот проект не заменяет полноценную модель угроз. Он дает простой воспроизводимый минимум: шифрование содержимого, шифрование имен файлов внутри архива и проверку целостности результата.
